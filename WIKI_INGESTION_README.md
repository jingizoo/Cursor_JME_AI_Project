...
### 2. **MediaWiki** (Optional API key)
- Generic MediaWiki-based wikis
- Supports custom wiki installations
- Example: `https://wiki.example.com/wiki/Page_Title`
...
### Example: With API Key (Private Wiki)
...
### "Failed to fetch MediaWiki page"
- Verify the wiki URL is accessible
- Check if API key is required (set `wiki_api_key`)
...
### MediaWiki
- `https://wiki.example.com/wiki/Page_Title`
